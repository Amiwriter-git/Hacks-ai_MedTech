...
model = CatBoostClassifier(iterations=300,learning_rate=1,depth=10)#SGDClassifier(loss="hinge", penalty="l2", max_iter=1500)#RandomForestClassifier(n_estimators=100, max_depth=100, max_features=30, bootstrap=False)#LogisticRegression(random_state=0)#SVC(gamma = 'auto')#SVC(gamma = 2, C = 1.0)#RandomForestClassifier(n_estimators=100)#KNeighborsClassifier(n_neighbors=5)
models.append(model)
        ...
        if rand_val not in rand_data:
            ...
            while m < (len(g)) and m_k < 10:
                ...
                if rand_val not in rand_data:
                    ...
                    if (len(rand_data) % 10 == 0) or (score >= good_level[i] and score_one >= good_level[i] and score_zero >= good_level[i]):
                        #print(f'Заболевание = {illness_list[i]}')
                        #print(g)
                        #print(f'{datetime.strftime(datetime.now(),"%d.%m.%Y_%H.%M.%S.%f")} - Перебрали вариантов = {len(rand_data)}, текущий вариант = {rand_val}, лучший вариант = {good_rand_val}, max_score = {max_score}')
                        #print(max_g)
